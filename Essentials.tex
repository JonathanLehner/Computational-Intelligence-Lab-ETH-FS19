...
\begin{compactdesc}
	...
	\begin{inparaitem}
		...
		\item $\|\mathbf{M}\|_F =\allowbreak \sqrt{\sum_{i=1}^{m} \sum_{j=1}^{n}\mathbf{m}_{i,j}^2} =\allowbreak \sqrt{\sum_{i=1}^{\min\{m, n\}} \sigma_i^2} = \sqrt{trace(A^\top A)}$
		\item $\|\mathbf{M}\|_1 = \sum_{i,j} | m_{i,j}|$  
		\item $\|\mathbf{M}\|_2 = \sigma_{\text{max}}(\mathbf{M})$
		\item $\|\mathbf{M}\|_{*} = \sum_{i=1}^{\min(m, n)} \sigma_i$ 
