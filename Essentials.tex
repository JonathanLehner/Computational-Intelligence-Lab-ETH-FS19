...
\begin{compactdesc}
	\item[Basics:]
		| $A\cdot B = C \Leftrightarrow c_{ik}=\sum_{j=1}^{m} a_{ij} \cdot b_{jk}, C=\sum_{i} a_i b_i^\top$

		| $A^\top\cdot B = C \Leftrightarrow c_{ik}=a_i^\top \cdot b_k$
		| $z=Ax\Leftrightarrow z_i=\sum_k a_{ik} x_k$
		| $z^\top = y^\top A \Leftrightarrow z_i = \sum_k a_{ki} y_k$
		| $A\cdot B=(A B^{(1)}\hspace{1pt} AB^{(2)}\hspace{1pt} \dots)=(A^{[1]}B\hspace{3pt}A^{[2]}B\hspace{3pt}\dots)^\top$
		| $\mathbf{Ker}(A)=\{x|Ax=0\}$, $\mathbf{Im}(A)=\{y|\exists x.Ax=y\}$
		| $\mathbf{Rank}(A) := dim(\mathbf{Im}(A))$
		| $\mathbf{Rank}(A\cdot B) \leq min(\mathbf{Rank}(A), \mathbf{Rank}(B))$
		| $dim(A)=\mathbf{Rank}(A) + dim(\mathbf{Ker}(A))$
	...
	\begin{inparaitem}
		...
		\item compl. to orth. proj.  $\mathbf{v} - \langle \mathbf{u},\mathbf{v} \rangle \mathbf{u}$
	...
	\begin{inparaitem}
		...
		\item $\|\mathbf{M}\|_F =\allowbreak \sqrt{\sum_{i=1}^{m} \sum_{j=1}^{n}\mathbf{m}_{i,j}^2} =\allowbreak \sqrt{\sum_{i=1}^{\min\{m, n\}} \sigma_i^2} = \sqrt{trace(M^\top M)}$
		\item $\|\mathbf{M}\|_1 = \sum_{i,j} | m_{i,j}|$  
		\item Spectral: $\|\mathbf{M}\|_2 = \sigma_{\text{max}}(\mathbf{M})$
		\item Nuclear: $\|\mathbf{M}\|_{*} = \sum_{i=1}^{\min(m, n)} \sigma_i$ 
		\item $\|\mathbf{M}\|_p = \max_{\mathbf{v} \neq 0} $ 
		\item $\frac{\|\mathbf{M}\mathbf{v}\|_p}{\|\mathbf{v}\|_p}$
		\item $\|\mathbf{M}\|_G = \sqrt{\sum_{i,j} \mathbf{g}_{ij} \mathbf{m}_{ij}^2}$
	\end{inparaitem}
\end{compactdesc}
...
Hessian: $\mathbf{H_{i,j}}=\frac{\partial^2 f}{\partial x_i \partial x_j}$ |
