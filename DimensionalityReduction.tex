...
For compression save $\mathbf{U}_k, \overline{\mathbf{Z}}_K, \overline{\mathbf{x}}$. $\mathbf{U}_k \in \mathbb{R}^{D \times K}, \boldsymbol{\Sigma} \in \mathbb{R}^{D \times D}, \overline{\mathbf{Z}}_K \in \mathbb{R}^{K \times N}, \overline{\mathbf{X}} \in \mathbb{R}^{D \times N}$ 
\textbf{Note:} $Var[z]=u^\top \Sigma u$ \textbf{err:} $\frac{1}{N}||\tilde{X}-\bar{X}||_F^2 = \sum_{i=K+1}^{D}\lambda_i$
